    ...
    if ($operation == 'add' || $operation == 'edit') {
        ...
        if ($operation == 'edit') {
            try {
                ...
                if (!$item_photo) {
                    $stmt = $conn->prepare("SELECT photo FROM tbl_setup_vehicle_parts WHERE id = :id");
                    $stmt->execute(['id' => $id]);
                ...
                $stmt = $conn->prepare("UPDATE tbl_setup_vehicle_parts SET vehicle_parts_name = :vehicle_parts_name, photo = :photo WHERE id = :id");
                $stmt->execute([
                    'vehicle_parts_name' => $vehicle_type,
                    'photo' => $item_photo,
                    'id' => $id
                ]);
                ...
                if (str_contains($e->getMessage(), 'vehicle_parts_name')) {
            ...
            try {
                ...
                $stmt = $conn->prepare("INSERT INTO tbl_setup_vehicle_parts(vehicle_parts_name, photo, added_by) 
                VALUES (:vehicle_parts_name, :photo, :added_by)");
                $stmt->execute([
                    'vehicle_parts_name' => $vehicle_type,
                    'photo' => $item_photo,
        ...
        if (isset($_POST['text_1'])) {
            $id = $_POST['text_1'];
            try {
                $stmt = $conn->prepare("UPDATE tbl_setup_vehicle_parts SET isDeleted = (CASE WHEN isDeleted = 1 THEN 0 ELSE 1 END) WHERE id = :id");
                $stmt->execute(['id' => $id]);
...
if (isset($_POST['tbl_1'])) {
    $stmt = $conn->prepare("SELECT * FROM tbl_setup_vehicle_parts");
    $stmt->execute();
