...
if (isset($_POST['operation'])) {
    ...
    if ($operation == 'add' || $operation == 'edit') {
        ...
        $manufacturer_name = (empty($_POST['text_2'])) ? NULL : strtoupper($_POST['text_2']);
        $uploaded_photo = isset($_FILES['text_4']) ? $_FILES['text_4'] : null;

        $upload_dir = '../images/manufacturer/'; // Directory to save images
        ...
        if ($operation == 'edit') {
            try {
                ...
                if (!$item_photo) {
                    $stmt = $conn->prepare("SELECT photo FROM tbl_setup_vehicle_manufacturers WHERE id = :id");
                    $stmt->execute(['id' => $id]);
                ...
                $stmt = $conn->prepare("UPDATE tbl_setup_vehicle_manufacturers SET manufacturer_name = :manufacturer_name, photo = :photo WHERE id = :id");
                $stmt->execute([
                ...
                if ($stmt) {
                    $output = ["success", "Success", "Manufacturer Details Updated."];
                } else {
                    $output = ["error", "Error", "Failed to update details."];
                }
            } catch (PDOException $e) {
                if (str_contains($e->getMessage(), 'manufacturer_name')) {
                    $output = ["error", "Error", "Manufacturer Already Saved"];
                } else {
            ...
            try {
                ...
                $stmt = $conn->prepare("INSERT INTO tbl_setup_vehicle_manufacturers(manufacturer_name, photo, added_by) 
                VALUES (:manufacturer_name, :photo, :added_by)");
                $stmt->execute([
                    'manufacturer_name' => $manufacturer_name,
                    'photo' => $item_photo,
                ...
                if ($stmt) {
                    $output = ["success", "Success", $manufacturer_name . " Successfully Added"];
                } else {
                    $output = ["error", "Error", "Failed to add Manufacturer."];
                }
            } catch (PDOException $e) {
                if (str_contains($e->getMessage(), 'manufacturer_name')) {
                    $output = ["error", "Error", "Manufacturer Already Saved"];
        ...
        if (isset($_POST['text_1'])) {
            $id = $_POST['text_1'];
            try {
                $stmt = $conn->prepare("UPDATE tbl_setup_vehicle_manufacturers SET isDeleted = (CASE WHEN isDeleted = 1 THEN 0 ELSE 1 END) WHERE id = :id");
                $stmt->execute(['id' => $id]);

                if ($stmt) {
                    $output = ["success", "Success", "Manufacturer Updated."];
                } else {
...
if (isset($_POST['tbl_1'])) {
    $stmt = $conn->prepare("SELECT * FROM tbl_setup_vehicle_manufacturers");
    $stmt->execute();
