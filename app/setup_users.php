            ...
            <form id="frm_1" class="frm_1" action="setup_users_act.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="div_driver">
                        <div class="form-group">
                            <label for="text_12">License Number</label>
                            <input type="text" name="text_12" id="text_12" class="form-control">
                        </div>
                        <div>
                            <div class="form-group">
                                <label for="text_13">License Expiry Date</label>
                                <input type="date" name="text_13" id="text_13" class="form-control">
                            </div>
                        </div>
                        <div>
                            <div class="form-group">
                                <label for="text_14">Contact No</label>
                                <input type="number" name="text_14" id="text_14" class="form-control">
                            </div>
                        </div>
                        <div>
                            <div class="form-group">
                                <label for="text_15">Address</label>
                                <textarea name="text_15" id="text_15"  class="form-control"></textarea>
                            </div>
                        </div>
                        <div>
                            <div class="form-group">
                                <label for="text_15">Vehicle Type</label>

                                <?php
                                    try {
                                        $stmtgetVehicleType = $conn->prepare("SELECT id, vehicle_type_name FROM tbl_setup_vehicle_types WHERE isDeleted = 0");
                                        $stmtgetVehicleType->execute();
                                        $vehicle_types_user = $stmtgetVehicleType->fetchAll(PDO::FETCH_ASSOC);
                                    } catch (PDOException $e) {
                                        die("Error fetching vehicle types: " . $e->getMessage());
                                    }
                                ?>
                                <label for="text_16">Vehicle Type</label>
                                <select name="text_16" id="text_16" class="form-control">
                                <option value="">Select Vehicle Type</option>
                                <?php foreach ($vehicle_types_user as $row): ?>
                                    <option value="<?= htmlspecialchars($row['id']) ?>"><?= htmlspecialchars($row['vehicle_type_name']) ?></option>
                                <?php endforeach; ?>
                        ...
                        <div>
                            <div class="form-group">
                                <label for="text_17">Plate No.</label>
                                <input type="text" name="text_17" id="text_17" class="form-control">
                            </div>
                        </div>
                    </div>
...
$(document).on("click", ".edit_btn", function() {
        ...
        const itemLicense_number =  rowData.row17; 
        const itemLicense_expiry =  rowData.row18;
        const itemContactno =  rowData.row19;  
        const itemAddress =  rowData.row20;  
        const itemvehicletype =  rowData.row21;  
        const itemvehicleplateno =  rowData.row22;  
        // const itemBranch =  rowData.row14; 
        // const itemBranch_name =  rowData.row15; 
        const itemImage = rowData.row3; // Get item image URL

        if(itemUsertype != 3) {
            $(".div_driver").addClass("d-none");
        }else {
            $(".div_driver").removeClass("d-none");
        }
        ...
        $('#text_12').val(itemLicense_number); 
        $('#text_13').val(itemLicense_expiry); 
        $('#text_14').val(itemContactno); 
        $('#text_15').val(itemAddress); 

        $('#text_16').val(itemvehicletype); 
        $('#text_17').val(itemvehicleplateno); 
